...
G= graphs[0]
#nx.from_numpy_matrix(graphs[0])

coords = pd.read_csv("./Neighshells/minRep_opt_2.xyz", delim_whitespace=True,
                     header=None, names=['atom', 'x','y','z'])

print(coords.head())
pos = coords[['x','y','z']]
pos_d = {}
for index, row in pos.iterrows():
    pos_d[index] = row[:2].values.tolist()


import matplotlib.pyplot as plt
black_edges = [edge for edge in G.edges() if edge != 0 ]


fig, ax = plt.subplots(dpi=300)
# nx.draw(G, pos=pos_d, node_color='purple', alpha=0.7, ax=ax)
nx.draw(G, pos=pos_d, with_labels=False, node_size=20, node_color='violet', alpha=0.7, ax=ax)  # draw nodes and edges
# nx.draw_networkx_labels(G, pos=pos_d)  # draw node labels/names
# draw edge weights
# labels = nx.get_edge_attributes(G, 'weight')
#nx.draw_networkx_edge_labels(G, pos_d, edge_labels=labels, s=20, ax=ax)
nx.draw_networkx_edges(G, pos_d, edgelist=black_edges, width=0.05, edge_color="gainsboro", alpha=0.7, ax=ax)
plt.axis("on")
#ax.set_xlim(0, 18)
#ax.set_ylim(0,18)
ax.set_xlabel("Y")
...
plt.savefig("NxGraph_crystal.png", dpi=200)
plt.show()
...
# nx.draw(G, pos , with_labels =False, width=0.4, 
#         node_color='lightblue', node_size=400)


# plt.figure(figsize=(10,6))
# oldpos = nx.spring_layout(G, k=0.8, scale=2)
# pos = nx.combinatorial_embedding_to_pos(oldpos)

# nx.draw_networkx_edges(G, pos, edge_color="gray", alpha=0.8)

# nx.draw(G,pos, with_labels=False, width=0.4, 
#         node_color='lightblue', node_size=10)#, with_labels=True)
