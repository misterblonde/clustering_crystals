...
def generate_subsets(pca, df_cat, clustering):
    ...
    qualitative_colors = sns.color_palette("Set3", 7, as_cmap=True)
    markers = ['o', 'v', '^','d', '*', 'X','.']#'>', 's','d', 'h', 'X','.']
    s0 = d[d['motif'] == 0 ]
    s1 = d[d['motif'] == 1 ]
    s2 = d[d['motif'] == 2 ]
    s5 = d[d['motif'] == 5 ]
...
feature = pd.read_csv(f'data/Motifs50.csv')
...
if clustering_algo == "Agglomerative":
    ...
    clustering = DBSCAN(eps=10, min_samples=1)
...
plt.tight_layout()
plt.savefig(f"{kernel_name}_{n_clusters}_PCA_{clustering_algo}_motifs50.png", dpi=300)
plt.show()
