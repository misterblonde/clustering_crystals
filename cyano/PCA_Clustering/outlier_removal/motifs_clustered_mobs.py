...
def generate_subsets(pca, df_cat, ener, mobs, clustering):
    """
    ...
    for i, r in df_cat.iterrows():
        ...
        d_clusters['mob'].append(test_merge.iloc[i]['MaxMob'])
...
plotting = "True"
print("Number of outliers rescaled: ", n_cut)
...
mobs = pd.read_csv(f"data/mobilities_{carrier}.csv", header=None, names=['ID',
                        'Jmax', 'MaxMob', 'av_ab', 'min_ab', 'max_ab',
                        'av_bc', 'min_bc', 'max_bc', 'av_ac', 'min_ac',
                        'max_ac'], index_col=False)
mobs["ID"] = mobs["ID"].astype("int64")
#df1.merge(df2, how='left', on='a')
test_merge = filns.merge(mobs, how='left', on='ID')

vmax = test_merge['MaxMob'].max()
vmin = test_merge['MaxMob'].min()
test_merge['MaxMob'] = test_merge['MaxMob'].fillna(10)
...
feature['packing'] = feature['packing'].fillna(8)
...
qualitative_colors2 = "plasma"#"spring"
markers = ['o', 'v', '^','>', 's','d', 'h', 'X','.']

s0, s1, s2, s3, s4, s5, s6, s7, s8, m = generate_subsets(pca, feature, ener, test_merge, clustering)

print("Minimum and maximum mob: ", vmin, vmax)
fig, ax = plt.subplots(figsize=(11.69,8.27)) #figure(figsize=(8.27, 11.69), dpi=100)
a0 = ax.scatter(s0.pca0, s0.pca1,marker=m[0], c=s0.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="Undefined")
a0.set_clim(vmin,vmax)
a0.cmap.set_over(Silver, alpha=0.2) #
a1 = ax.scatter(s1.pca0, s1.pca1,marker=m[1], c=s1.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="T")
a1.set_clim(vmin,vmax)
a1.cmap.set_over(Silver, alpha=0.2)#
a2 = ax.scatter(s2.pca0, s2.pca1,marker=m[2], c=s2.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="B")
a2.set_clim(vmin,vmax)
a2.cmap.set_over(Silver, alpha=0.2) #
a3 = ax.scatter(s3.pca0, s3.pca1,marker=m[3], c=s3.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="I")
a3.set_clim(vmin,vmax)
a3.cmap.set_over(Silver, alpha=0.2) 
a4 = ax.scatter(s4.pca0, s4.pca1,marker=m[4], c=s4.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="T,B" ) 
a4.set_clim(vmin,vmax)
a4.cmap.set_over(Silver, alpha=0.2) 
a5 = ax.scatter(s5.pca0, s5.pca1,marker=m[5], c=s5.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="T,I")
a5.set_clim(vmin,vmax)
a5.cmap.set_over(Silver, alpha=0.2) 
a6 = ax.scatter(s6.pca0, s6.pca1,marker=m[6], c=s6.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="B,I" ) 
a6.set_clim(vmin,vmax)
a6.cmap.set_over(Silver, alpha=0.2) 
a7 = ax.scatter(s7.pca0, s7.pca1,marker=m[7], c=s7.mob, s=35, alpha=0.75, cmap=qualitative_colors2, label="T,B,I")
a7.set_clim(vmin,vmax)
a7.cmap.set_over(Silver, alpha=0.2) 
a8 = ax.scatter(s8.pca0, s8.pca1,marker=m[8], c=s8.mob, s=35, alpha=0.5, cmap=qualitative_colors2, label="Undefined")
a8.set_clim(vmin,vmax)
a8.cmap.set_over(Silver, alpha=0.5) 
...
# plt.tight_layout()
# plt.colorbar()
cbar = plt.colorbar(a0,ax=ax, orientation='horizontal', cmap=qualitative_colors2, spacing='proportional')#,ticks=ticks)
cbar.set_label(f'Maximum {carrier} mobility', fontsize=14)
plt.savefig(f"{kernel_name}_{n_clusters}_{linkage_criterium}_{clustering_algo}_{n_cut}_c_mobs.png", dpi=300)
# plt.show()
