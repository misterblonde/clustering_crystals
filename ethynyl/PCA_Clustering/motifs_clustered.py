...
def generate_subsets(pca, df_cat, ener, clustering):
    ...
    for i, r in df_cat.iterrows():
        ...
        d_clusters['pca0'].append(pca.components_[1][i])
        d_clusters['pca1'].append(pca.components_[2][i])
...
kernel_name = "PM_4_6"
clustering_algo = str(sys.argv[3])
linkage_criterium = str(sys.argv[2])
# read in data:
...
duplicates = [item for item in flat[:1]]
...
if clustering_algo == "Agglomerative":
    ...
    print(n_clusters, min(list(X_fitted.distances_)), max(X_fitted.distances_), mean(list(X_fitted.distances_)) )
...
for idx, name in enumerate(pca.components_[1]):
    ...
    if E_rank_label <= 25: # E_rank_label energy ranking label
            texts.append(plt.text(xval,yval, W99_label, weight="bold", fontsize=10)) # 

    if name < -0.10 or name > 0.15 or xval > 0.072: #name > 0.12 or
