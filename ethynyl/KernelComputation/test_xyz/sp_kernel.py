...
# ‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹ USER INPUT ››››››››››››››››››››››››››››››››››››››››››››››››››
inp_folder = 'xyz'#Neighshells'   # dir with xyz to convert into graphs
...
def real_run(kernel_name, kernel, graphs, names_list):
    ...
    for i, g in enumerate(graphs_list):
        ...
        # make ref graph 
        # print(i)
        ref_kernel = pm.fit_transform([g])
        row_list = []
        similarity = pm.transform(graphs_list)
        print(type(similarity))
        print(similarity.flat[0])
        # print(np.shape(similarity))
        matrix.append(similarity)
        # for j, graph in enumerate(list(graphs)):
        #     similarity = pm.transform(graph)
        #     row_list.append(similarity.flat[0])
            # print(f"{names_list[idx]}\t{similarity.flat[0]}")

        # matrix.append(row_list)
        end3 = time.time()
        print("Time needed for Kernel comparison ROW (s): ", end3 - start3)
