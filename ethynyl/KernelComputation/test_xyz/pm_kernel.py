...
# ‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹ USER INPUT ››››››››››››››››››››››››››››››››››››››››››››››››››
inp_folder = 'Neighshells'   # dir with xyz to convert into graphs
...
def real_run(kernel_name, kernel, graphs, node_attrs, names_list):
    pm = kernel
    # start = time.time()
    # ref_kernel = pm.fit_transform(ref_graph)
    # end = time.time()
    ...
    for i, g in enumerate(graphs):
        print(i)
        # make ref graph 
        ref_graph = read_in_grakel(g, node_attrs[i])
        ref_kernel = pm.fit_transform(ref_graph)
        row_list = []
        for idx, graph in enumerate(graphs):
            grakel_graph = read_in_grakel(graph, node_attrs[idx])

            similarity = pm.transform(grakel_graph)
            row_list.append(similarity.flat[0])
...
print(L, d)
pm = PyramidMatch(L=L, d=d)
real_run(f"PM_{L}_{d}", pm, graphs, node_attrs, names_list)
...
sp = ShortestPath()
real_run(f"SP", sp, graphs, node_attrs, names_list)
