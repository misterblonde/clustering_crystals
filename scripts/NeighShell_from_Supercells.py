...
for mol in os.listdir(f"{inp_folder}/"):
    ...
    if end_file.is_file():
        ...
        print(f"{int(len(labels)/int(n_atoms))} molecules in supercell.")

        # init
        counter = 0
        distance_dict= {}

        for j in range(1,int(len(labels)/n_atoms)):

            index_start = int(n_atoms)*j
            index_end = int(n_atoms)*(j+1)


            coordfile1 = np.genfromtxt(f"{inp_folder}/{mol}")[0:n_atoms,1:]
            ...
            if distance<cutoff:
                ...
                counter += 1
                #print(f"Mol {j} ", round(distance, 2))

            #else: #print("Mol out of range")
